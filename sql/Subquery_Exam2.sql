...
select b.bookname, b.price, o.saleprice, b.price-o.saleprice as gap
from orders o, book b, customer c
where o.bookid = b.bookid and o.custid = c.custid
and c.name = '박지성'
;
...
-- 2 마당서점의 운영자와 경영자가 요구하는 다음 질문에대해 SQL 문을 작성하시오.

-- (8) 주문하지 않은 고객의 이름(부속질의 사용)
-- 주문한 고객을 찾고 반전 or exist 사용
-- 구매를 한 고객의 번호
...
-- ㄴ> select의 결과가 1행이라도 있으면 참, ( ) 안에 상관관계가 있어야 함
;
...
;
-- 1. join
-- 2. group by => 컬럼 2개 사용 -> name은 중복 가능하기 때문에 ~> custid 사용
...
-- 고객별 총 판매액 출력 조건 -> 총 평균보다 더 높은 값을 가지는 고객만 찾기
...
select avg(saleprice) from orders; -- 평균

-- 3. 마당서점에서 다음의 심화된 질문에 대해 SQL 문을 작성하시오.

-- (1) 박지성이 구매한 도서의 출판사와 같은 출판사에서 도서를 구매한 고객의 이름
-- orders, book => publisher in (서브쿼리)
-- 박지성이 구매한 도서의 출판사 이름
...
-- 고객별 구매한 도서의 출판사의 수 -> having 절
select c.custid, c.name, count(distinct b.publisher) as "출판사의 수"
from orders o, book b, customer c
where o.bookid = b.bookid and o.custid = c.custid
group by c.custid, c.name

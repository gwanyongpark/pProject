...
-- 서브 쿼리를 이용해서 기존 테이블의 구조로 테이블을 생성, 데이터도 저장가능
-- 제약조건은 복사되지 않는다.
...
-- 원하는 컬럼만 혹은 원하는 로우만 선택해서 테이블 생성 가능
...
-- 테이블의 변경 : 저장구조(스키마의 구조 : 컬럼들의 집합)가 변경된 것
-- 컬럼 변경(테이블에 컬럼을 추가하는 것, 테이블에 있는 컬럼을 삭제하는 것, 컬럼의 내용이나 이름을 수정하는 것)
...
-- 테이블 객체의 삭제
-- drop table 테이블 이름 => 스키마 삭제와 저장데이터 삭제(모든 로그삭제)
...
-- check : 특정 범위 혹은 값을 확인 후 입력 또는 수정해주는 제약
...
insert into emp01 (empno, ename, job,   sal, gender, deptno) -- hiredate X

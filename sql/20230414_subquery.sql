...
-- 스칼라 부속질의 : select 절에 쓸 수 있는 서브쿼리, 단일값(질의의 결과 : 단일행 단일열)
select deptno, (select dname from dept where dept.deptno = emp.deptno) as "부서이름"
from emp
where ename = 'SCOTT'
;
...
select o.custid, sum(saleprice) as total -- 영문 쓸 땐 as 생략가능
...
-- 중첩 질의 : where 절 뒤에 오는 비교연산 혹은 다른 연산자를 통해 처리할 때 사용

-- 비교 연산자 : 단일 값이 나오는 서브쿼리를 사용해야 한다
-- 평균급여보다 더 많은 급여를 받는 사원을 찾아보자
...
-- join으로 풀이
-- 대한민국에 거주하는 고객에게 -> costomer
-- 판매한 도서의 총 판매액을 구하시오. -> orders

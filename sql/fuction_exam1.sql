...
select 
    ...
    substr(hiredate, 4,2) as "입사 한달"
from emp;
--where substr(hiredate, 1,2) = 81; <- 81년도 입사자만 추출
...
select ename, 
    ...
    to_char(hiredate, 'dy') as "입사 일",
    to_char(hiredate, 'yy.mm.dy') as "입사"
from emp;
...
-- 관리자를 알 수 없는 사원(<- mgr이 null)과 최저 급여가 2000 미만인 그룹은 제외시키고(having) 
...
select deptno, count(*), round(avg(sal),2) as "평균 급여" 
from emp 
group by deptno
;

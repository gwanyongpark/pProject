...
public class BoardWriteService {
    ...
    public int writeBoard(
        ...
        if (board.getFile() !=null && board.getFile().getSize()>0){
            ...
            // 새로운 파일 이름 생성 : 중복된 데이터 이름을 중복되지 않도록 처리
            String newFileName = UUID.randomUUID().toString() + board.getFile().getOriginalFilename(); // <- 난수 + 파일 이름

            File newFile = new File(realPath, newFileName);
            try {
                // 파일 저장
                board.getFile().transferTo(newFile);
                // RequestRegBoard filename 에 새로운 파일 이름 저장
